...
import { Upload, FileCheck, RotateCcw, CheckCircle, AlertTriangle, XCircle, ArrowLeft, Loader2, ListChecks, PieChart } from 'lucide-react';
...
export default function ImportBudgetPage() {
        ...
        Papa.parse(file, {
            ...
            complete: (results: ParseResult<Record<string, string>>) => {
                ...
                if (!results.data.length || !results.meta.fields?.length) {
                 ...
                 if (!headers.includes('category') || !headers.includes('description') || !headers.includes('amount')) {
                    setImportError("Missing required CSV headers: category, description, amount.");
                    setStage('error');
                    return;
                 }
                ...
                proceedToPreview(dataWithIndex); // Directly proceed to preview after parsing
            },
            error: (error: Error) => {
    ...
    const proceedToPreview = (parsedRows: ParsedRow[]) => {
        const mapped: MappedBudgetItem[] = parsedRows.map((row) => {
            const budgetItem: Partial<MappedBudgetItem> & { amount?: number } = {
                __originalData: row, __toBeImported: true, description: '', amount: undefined, category: 'recurring-expense',
            };
            let parseError = '';
            ...
            if (categoryValidation.success) {
                if (categoryValidation.data === 'unplanned-expense') {
                    ...
                    budgetItem.category = categoryValidation.data as BudgetItemCategory;
                }
            } else {
            ...
            if (isNaN(parsedAmount) || parsedAmount <= 0) {
                parseError += `Invalid or non-positive amount: '${row.amount || ""}'. `;
            } else {
                budgetItem.amount = parsedAmount;
            }


            if (parseError) { budgetItem.__parseError = parseError.trim(); budgetItem.__toBeImported = false; }
    ...
    const handleConfirmImport = () => {
        ...
        setImportError(null);
        const itemsToImport = mappedBudgetItems.filter(item => item.__toBeImported && !item.__parseError);
        ...
        if (itemsToImport.length === 0) {
            setImportedCount(0);
            setStage('complete');
            toast({ title: "Import Complete", description: `No new budget items were imported. ${currentSkippedCount} rows skipped.`, variant: "default" });
            return;
        }
        // The store action `importBudgetsBatch` expects Omit<BudgetItem, 'id' | 'period'>
        const newBudgetData: Omit<BudgetItem, 'id' | 'period'>[] = itemsToImport.map(item => ({
        ...
        try {
            ...
            setStage('complete');
            toast({ title: "Import Successful", description: `${addedItems.length} budget items imported to period ${format(parse(budgetPeriod, 'yyyy-MM', new Date()), 'MMMM yyyy')}. ${currentSkippedCount} rows skipped.`, variant: "default" });
        } catch (error: any) {
            setImportError(`Failed to save budget items: ${error.message}`);
            setStage('error');
             toast({ title: "Import Failed", description: "Could not save imported budget items.", variant: "destructive" });
        }
    };

    const resetState = () => {
        setStage('upload'); setFile(null); setFileName(''); setIsParsing(false); setParsedData([]);
        setMappedBudgetItems([]); setImportError(null); setImportedCount(0); setSkippedCount(0);
        const fileInput = document.getElementById('file-upload') as HTMLInputElement;
        if (fileInput) fileInput.value = '';
    };

    const handleBack = () => {
        if (stage === 'preview') setStage('upload'); // No mapping stage for budget simplified version
    ...
    const renderUploadStage = () => (
        <Card>
            <CardHeader className="p-6">
                <CardTitle className="text-lg flex items-center gap-2"><PieChart className="h-5 w-5 text-primary" />Import Budget Items (Step 1/3)</CardTitle>
                <CardDescription>Select a CSV file with columns: `category`, `description`, `amount`.</CardDescription>
            </CardHeader>
         ...
         <Card>
            <CardHeader className="p-6">
                <CardTitle className="text-lg">Preview Budget Items (Step 2/3)</CardTitle>
                 <CardDescription>Review parsed items for period: <span className='font-semibold'>{format(parse(budgetPeriod, 'yyyy-MM', new Date()), 'MMMM yyyy')}</span>. Uncheck rows to exclude. Rows with errors cannot be imported.</CardDescription>
             </CardHeader>
                     ...
                     <Table>
                         ...
                         <TableBody>
                             {mappedBudgetItems.map((item, index) => (
                                 <TableRow
                                    key={index}
                                    className={cn(item.__parseError && "bg-destructive/10 text-destructive")}
                                    ...
                                    <TableCell className="text-center pl-6 pr-3">
                                     ...
                                     <TableCell className="capitalize px-3">{item.category}</TableCell>
                                     <TableCell className="max-w-[200px] truncate px-3">{item.description}</TableCell>
                                     <TableCell className={`text-right font-mono px-3 ${item.amount === undefined || isNaN(item.amount) ? 'text-destructive italic' : ''}`}>
                                        {item.amount !== undefined && !isNaN(item.amount) ? formatCurrency(item.amount) : 'Invalid'}
                                     </TableCell>
                                     <TableCell className="text-xs pr-6 pl-3">
                                         {item.__parseError ? <span className="flex items-center gap-1 text-destructive"><XCircle size={14} /> Error</span> :
             ...
             <CardFooter className="flex flex-col sm:flex-row justify-between items-center gap-2 p-6">
                <Button variant="outline" onClick={handleBack}>
                 ...
                 <div className='text-sm text-muted-foreground text-center sm:text-left'>
                    {mappedBudgetItems.filter(item => item.__toBeImported && !item.__parseError).length} of {mappedBudgetItems.length} rows selected for import.
                 </div>
                 <Button onClick={handleConfirmImport} disabled={mappedBudgetItems.filter(item => item.__toBeImported && !item.__parseError).length === 0}>
     ...
     const renderReconcilingStage = () => (
         <Card>
             <CardHeader className="p-6">
                 ...
                 <CardDescription>Please wait while the selected items are being added to {format(parse(budgetPeriod, 'yyyy-MM', new Date()), 'MMMM yyyy')}.</CardDescription>
             </CardHeader>
      ...
      const renderCompleteStage = () => (
         <Card>
             <CardHeader className="p-6">
                 ...
                 <CardDescription>The budget item import process has finished.</CardDescription>
             </CardHeader>
             <CardContent className="space-y-2 p-6">
                 <p>Successfully imported: <span className="font-semibold">{importedCount}</span> items to budget for {format(parse(budgetPeriod, 'yyyy-MM', new Date()), 'MMMM yyyy')}.</p>
      ...
      const renderErrorStage = () => (
         <Card className="border-destructive">
             <CardHeader className="p-6">
                 ...
                 <CardDescription>An error occurred during the budget import process.</CardDescription>
             </CardHeader>
