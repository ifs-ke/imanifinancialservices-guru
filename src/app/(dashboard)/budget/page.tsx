...
import { Input } from '@/components/ui/input'; // Kept for search if added later
...
export default function BudgetPage() {
  ...
  const { deleteBudgetItem } = useBudgetStore(); // importBudgetsBatch removed
  ...
  useEffect(() => {
      // Set initial budget period based on current month or selectedMonthDate
      const initialPeriod = formatToPeriodKey(selectedMonthDate);
      if (budgetPeriod !== initialPeriod) { // Only set if different to avoid loop
  ...
  const groupedBudgetItems = useMemo(() => {
      ...
      budgetItemsForPeriod.forEach(item => {
          // Ensure item.category is valid before accessing groups
  ...
  const handleExport = () => {
    ...
    // Exclude id and period from export
    ...
    const csvData = Papa.unparse(dataToExport, {
      header: true,
       columns: ['category', 'description', 'amount'] // Specify column order
    });
         ...
         {budgetCategories.map(({ name, key, icon: Icon }) => (
             <Card key={key} className={cn("flex flex-col shadow-sm", key === 'debt' && 'lg:col-span-1 xl:col-span-1')}>
                 <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 border-b p-4">
                     ...
                     <Button variant="ghost" size="sm" onClick={() => handleAddClick(key as BudgetItemCategory)} className="h-7 px-2"><PlusCircle className="mr-1 h-3 w-3" />Add {key === 'debt' ? 'Allocation' : name.replace(/s$/, '')}</Button>
                 </CardHeader>
                  ...
                  {groupedBudgetItems[key as BudgetItemCategory].length > 0 && (
                      <CardFooter className="p-4 border-t text-sm">
                          <div className="flex justify-between w-full"><span className="font-semibold">Total {name}</span><span className="font-bold font-mono">{formatCurrency(groupTotals[key as BudgetItemCategory])}</span></div>
                      </CardFooter>
                  )}
             </Card>
         ))}
