...
export async function POST(request: Request) {
  ...
  const userId = process.env.NEXT_PUBLIC_MOCK_USER_ID; // Use mock user ID
  
  const logContextBase = { userId: userId || 'unknown-save-post', operation: 'POST /api/save', apiRoute: '/api/save' };

  if (!userId) {
    logWarn('Save API: Unauthorized save attempt: No MOCK_USER_ID set or user not logged in.', logContextBase, userId);
    const response = NextResponse.json({ error: 'Unauthorized: User not identified.' }, { status: 401 });
