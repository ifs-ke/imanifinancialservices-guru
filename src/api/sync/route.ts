    ...
    return dataArray.map((item: any) => {
        if (item.date && !(item.date instanceof Date)) {
            try {
                const parsedDate = new Date(item.date);
                 ...
                 // // console.warn(`Sync API: Invalid date format encountered in DB for ${collectionName}, item ID ${item.id || 'N/A'}, user ${userId}. Defaulting date.`, { itemDateValue: item.date }); // Console log commented out
        ...
        if (item.timestamp && !(item.timestamp instanceof Date)) {
             try {
                 ...
                 // // console.warn(`Sync API: Invalid timestamp format encountered in DB for ${collectionName}, item ID ${item.id || 'N/A'}, user ${userId}. Defaulting timestamp.`, { itemTimestampValue: item.timestamp }); // Console log commented out
     ...
     // // console.error(`Sync API: Error fetching ${collectionName} for user ${userId}:`, { error, stack: error instanceof Error ? error.stack : undefined }); // Console log commented out
    return [];
  }
}
      ...
      for await (const doc of ownedReviewsCursor) {
          if (doc.weekKey && typeof doc.weekKey === 'string') { // Ensure weekKey is a string
             ...
             reviewsMap[doc.weekKey] = doc as WeeklyReviewData;
              ...
              // // console.warn(`Sync API: Found owned review with invalid or missing weekKey for user ${userId}. Skipping.`, { docId: doc._id }); // Console log commented out
          }
      }
       // // console.log(`Sync API: Fetched ${Object.keys(reviewsMap).length} owned weekly reviews`, logContext); // Console log commented out
       return reviewsMap;
    } catch (error) {
       // // console.error(`Sync API: Error fetching owned weeklyReviews for user ${userId}:`, { error, stack: error instanceof Error ? error.stack : undefined }); // Console log commented out
       return {};
    }
}
      ...
      const collection = db.collection('weeklyReviews');
       ...
       for await (const doc of sharedReviewsCursor) {
           if (doc.weekKey && typeof doc.weekKey === 'string') { // Ensure weekKey is a string
              ...
              // // console.warn(`Sync API: Found shared review with invalid or missing weekKey shared with user ${userId}. Skipping.`, { docId: doc._id, ownerId: doc.ownerId }); // Console log commented out
           }
       }
        // // console.log(`Sync API: Fetched ${Object.keys(reviewsMap).length} shared weekly reviews`, logContext); // Console log commented out
       return reviewsMap;
    } catch (error) {
       // // console.error(`Sync API: Error fetching shared weeklyReviews for user ${userId}:`, { error, stack: error instanceof Error ? error.stack : undefined }); // Console log commented out
       return {};
    }
}
...
async function getUserProfileData(db: any, userId: string): Promise<{ startDate?: string, endDate?: string, gettingStartedDismissed?: boolean }> {
    ...
    try {
        ...
        // // console.error(`Sync API: Error fetching user profile data for user ${userId} from collection '${collectionName}':`, { error, stack: error instanceof Error ? error.stack : undefined }); // Console log commented out
...
export async function GET() {
  ...
  try {
    ...
    const [
        ...
        getOwnedWeeklyReviews(db, userId), // Pass actual userId
        getSharedWeeklyReviews(db, userId), // Pass actual userId
        getUserProfileData(db, userId), // Pass actual userId
        getCollectionData<NotificationItem>(db, 'notifications', userId) // Fetch notifications for the user
    ]);
