...
export function useSyncManager() {
  ...
  const clearLocalState = useCallback(() => {
    ...
    logInfo('SyncManager: Clearing local state.', undefined, contextUserId);
    ...
    try {
      ...
      storeKeys.forEach(key => {
        try { 
          ...
          logWarn(`Failed to remove ${key} from sessionStorage`, { error: e }, contextUserId); 
        }
      });

      logInfo('SyncManager: Local state cleared.', undefined, contextUserId);
      updateSyncState({ 
        status: 'local',
        lastSyncTime: null,
        gettingStartedDismissed: false,
      ...
      logError('Error during clearLocalState', error, undefined, contextUserId);
    } finally {
  ...
  const fetchData = useCallback(async (isRetry = false, skipHashCheck = false) => {
    if (!isClerkLoaded) {
      logDebug('Fetch Aborted: Auth not loaded yet.', undefined, userId);
      return false;
    }
    if (!isSignedIn || !userId) {
      logWarn('Fetch Aborted: User not signed in or userId not available.', { isSignedIn }, userId);
    ...
    logInfo(`Fetch Triggered${isRetry ? ' (Retry)' : ''}${skipHashCheck ? ' (Skip Hash Check)' : ''}...`, undefined, userId);
    ...
    try {
      ...
      if (abortControllerRef.current?.signal.aborted) {
        logDebug('Fetch Aborted: Operation was cancelled by cleanup.', { reason: abortControllerRef.current?.signal.reason }, userId );
        updateSyncState({ status: 'local' });
        return false;
      }
      ...
      if (!skipHashCheck && dataHash) {
        ...
        if (!isValid) {
          logError('Fetch Error: Data integrity check failed during fetch!', { serverHash: dataHash, clientHashCalculationInputTruncated: dataString.substring(0, 200) }, userId );
      ...
      logDebug('Fetch: Operation complete.', undefined, userId);
    }
  }, [
  ...
  const saveData = useCallback(async (isForceSave = false) => {
    if (!isClerkLoaded) { logDebug('Save Aborted: Auth not loaded yet.', undefined, userId ); return false; }
    if (!isSignedIn || !userId) { logWarn('Save Aborted: User not signed in or userId not available.', { isSignedIn }, userId ); updateSyncState({ status: 'local' }); return false; }
    ...
    logInfo(`Save Triggered${isForceSave ? ' (Force)' : ''}...`, undefined, userId);
    updateSyncState({ status: 'syncing' });
    isSavingRef.current = true;

    if (!isForceSave) {
      logInfo('Save: Fetching latest data before saving to check for conflicts...', undefined, userId);
      const preSaveFetchSuccess = await fetchData(false, false); 
      if (!preSaveFetchSuccess) {
        logError('Save Aborted: Pre-save fetch failed or hash mismatch detected.', undefined, { operationStatus: 'pre-save-fetch-failed' }, userId);
      ...
      if(syncState.hashMismatch) {
        logError('Save Aborted: Hash mismatch detected after pre-save fetch.', undefined, undefined, userId);
        isSavingRef.current = false;
        return false;
      }
      logInfo('Save: Pre-save fetch successful, proceeding with save.', undefined, userId);
    } else {
      updateSyncState({ hashMismatch: false, isMismatchDialogOpen: false });
      logInfo('Save: Force save initiated, skipping pre-fetch check, proceeding with save.', undefined, userId);
    }
    ...
    try {
      ...
      logDebug(`Save Client: Calculated client hash: ${dataHash}`, undefined, userId);
      ...
      if (abortControllerRef.current?.signal.aborted) {
        logDebug('Save Aborted: Operation was cancelled by cleanup.', { reason: abortControllerRef.current?.signal.reason }, userId);
        updateSyncState({ status: 'local' }); 
        return false;
      }
      ...
      if (!response.ok) {
        ...
        if (response.status === 400 && errorPayload.message?.includes('integrity check failed')) {
          logError('Save API Error 400: Data integrity check failed on server.', errorPayload, undefined, userId);
          updateSyncState({ status: 'error', hashMismatch: true, isMismatchDialogOpen: true });
          ...
          logError('Save API Error 401: Unauthorized.', errorPayload, undefined, userId);
          updateSyncState({ status: 'error' });
          ...
          logWarn('Save API Error 429: Rate limit exceeded.', undefined, userId);
          updateSyncState({ status: 'error' });
          toast({ title: 'Save Failed: Too Many Requests', description: "Please wait a moment and try saving again.", variant: 'destructive' });
        } else {
          logError(`Save API Error ${response.status}: ${errorPayload.message}`, errorPayload, undefined, userId);
          updateSyncState({ status: 'error' });
          throw new Error(errorPayload.message, { cause: errorPayload });
        }
        return false;
      ...
      hasLocalChangesRef.current = false;
      logInfo(`Save Successful. Server: ${result.message}`, undefined, userId);
      toast({ title: 'Data Saved', description: 'Changes saved to cloud.' });
      return true;
    } catch (error: any) {
      if (error.name === 'AbortError') {
        logDebug(`Save Aborted: ${error.message}`, undefined, userId);
        updateSyncState(prev => ({ ...prev, status: prev.status === 'syncing' ? 'local' : prev.status }));
        return false;
      }
      logError('Save Error:', error, { cause: error.cause }, userId);
      updateSyncState({ status: 'error' });
      ...
      logDebug('Save: Operation complete.', undefined, userId);
    }
  }, [
    isSignedIn, userId, isClerkLoaded, toast,
    getTransactionsState, getDebtState, getStatementState, getBudgetState,
  ...
  const triggerDebouncedSave = useCallback(() => {
    if (!isSignedIn || !userId) { logWarn('Debounced Save: User not signed in. Save will not occur.', undefined, userId); return; }
    if (syncState.hashMismatch) { logWarn('Debounced Save: Blocked by hash mismatch. Manual resolution required.', undefined, userId); updateSyncState({ status: 'error' }); return; }
    cleanupAsyncOperations(`Starting new debounced save for user ${userId}`);
    logDebug('Debounced Save: Timer started/reset.', undefined, userId);
    saveTimeoutRef.current = setTimeout(() => { logInfo('Debounced Save: Timeout reached. Initiating save.', undefined, userId); saveData(); }, 3000);
  ...
  const handleStoreChange = useCallback(() => {
    ...
    if (!hasLocalChangesRef.current) logInfo('Store Change: First local change detected since last sync/load.', undefined, userId);
    hasLocalChangesRef.current = true;
    if (syncState.status === 'synced' || syncState.status === 'idle' || (syncState.status === 'error' && !syncState.hashMismatch) ) {
      updateSyncState({ status: 'local' });
      logInfo('Store Change: Status changed to "local" due to store changes.', { previousStatus: syncState.status }, userId);
    }
    triggerDebouncedSave();
  }, [syncState.status, syncState.hashMismatch, triggerDebouncedSave, userId, updateSyncState]);

  useEffect(() => {
    if (!isClerkLoaded) { logDebug('Auth Effect: Auth state not ready. Waiting for load.', undefined, userId); updateSyncState({ status: 'idle' }); return; }
    const currentAuthUserId = userId;
    if (currentAuthUserId && currentAuthUserId !== internalPreviousUserId.current) {
      logInfo(`Auth Effect: User signed in or changed. Current: ${currentAuthUserId}, Previous: ${internalPreviousUserId.current ?? 'none'}.`, { oldUserId: internalPreviousUserId.current, newUserId: currentAuthUserId }, currentAuthUserId);
      ...
      logInfo(`Auth Effect: User signed out. Was: ${internalPreviousUserId.current}.`, { oldUserId: internalPreviousUserId.current }, internalPreviousUserId.current);
      ...
      logInfo('Auth Effect: Same user session, initial fetch not completed. Triggering fetch...', undefined, currentAuthUserId);
      fetchData();
    } else if (!currentAuthUserId && !internalPreviousUserId.current && !initialFetchDoneRef.current) {
      logInfo('Auth Effect: Initial load, no active user session. Setting status to local.', undefined, userId);
      updateSyncState({ status: 'local' });
      initialFetchDoneRef.current = true; 
    } else {
  ...
  useEffect(() => {
    if (!isClerkLoaded || !isSignedIn || !userId || !initialFetchDoneRef.current) {
      logDebug('Change Subscription: Conditions not met (auth not ready or initial fetch not done).', { isClerkLoaded, isSignedIn, initialFetchDone: initialFetchDoneRef.current }, userId);
      return () => cleanupAsyncOperations('Store subscription cleanup (early return)');
    }
    if (syncState.hashMismatch) {
      logWarn('Change Subscription: Blocked due to hash mismatch. Data is local but potentially conflicting.', undefined, userId);
    ...
    logDebug('Change Subscription: Subscribing to store changes...', undefined, userId);
    ...
    return () => {
      logDebug('Change Subscription: Unsubscribing from store changes.', undefined, userId);
  ...
  useEffect(() => {
    if (!isClerkLoaded || !isSignedIn || !userId || !initialFetchDoneRef.current) {
      logDebug('Getting Started Tracker: Conditions not met or hash mismatch prevents update.', { isClerkLoaded, isSignedIn, initialFetchDone: initialFetchDoneRef.current, hashMismatch: syncState.hashMismatch }, userId);
      return;
    }
    if (syncState.hashMismatch) {
      logWarn('Getting Started Tracker: Change detected, but blocked by hash mismatch.', { gettingStartedDismissed: syncState.gettingStartedDismissed }, userId);
      return;
    }
    if (initialFetchDoneRef.current && !isFetchingRef.current && !isSavingRef.current && !isClearingRef.current) {
      logDebug('Getting Started Tracker: Change detected.', { gettingStartedDismissed: syncState.gettingStartedDismissed }, userId);
      hasLocalChangesRef.current = true;
      if (syncState.status === 'synced' || syncState.status === 'idle' || (syncState.status === 'error' && !syncState.hashMismatch) ) {
        updateSyncState({ status: 'local' });
        logInfo('Getting Started Tracker: Status changed to "local" due to dismissal state change.', { previousStatus: syncState.status }, userId);
      }
      triggerDebouncedSave();
    } else {
      logDebug('Getting Started Tracker: Dismissal change detected, but conditions prevent status update or already local.', { initialFetchDone: initialFetchDoneRef.current, isFetching: isFetchingRef.current, isSaving: isSavingRef.current, isClearing: isClearingRef.current, hashMismatch: syncState.hashMismatch, currentStatus: syncState.status }, userId);
    }
  ...
  const forceSaveLocal = useCallback(async () => {
    ...
    logWarn('SyncManager: User chose to force save local data, overwriting server.', undefined, userId);
    ...
    if (success) {
      ...
      logInfo('Force Save Local: Successful.', undefined, userId);
    } else {
      logError('Force Save Local: Failed.', undefined, undefined, userId);
  ...
  const forceFetchServer = useCallback(async () => {
    ...
    logWarn('SyncManager: User chose to force fetch server data, discarding local changes.', undefined, userId);
    ...
    if (success) {
      ...
      logInfo('Force Fetch Server: Successful.', undefined, userId);
    } else {
      logError('Force Fetch Server: Failed.', undefined, undefined, userId);
      if (syncState.hashMismatch) updateSyncState({ isMismatchDialogOpen: true }); 
    }
    return success;
  ...
  const retrySync = useCallback(() => {
    ...
    if (syncState.status === 'error' && syncState.hashMismatch) {
      logWarn('Manual Retry: Hash mismatch detected. Opening resolution dialog.', undefined, userId);
      updateSyncState({ isMismatchDialogOpen: true });
      return;
    }
    if (hasLocalChangesRef.current || (syncState.status === 'error' && !syncState.hashMismatch)) {
      logInfo('Manual Sync: Local changes or non-mismatch error. Attempting save...', undefined, userId);
      saveData();
    } else { 
